...
namespace Dfinance.Purchase.Services
{
    public class PurchaseQuotationService:IPurchaseQuotationService
    {
        ...
        public CommonResponse CancelPurchaseQuotation(int TransId, int PageId,string reason)
        {
            if (!_authService.IsPageValid(PageId))
            {
                return PageNotValid(PageId);
            ...
            if (!_authService.UserPermCheck(PageId, 5))
            {
                return PermissionDenied("Cancel Purchase Quotation");
            }
            try
            {
                var result = _transactionService.CancelTransaction(TransId,reason);
                _logger.LogInformation("Cancel of Purchase Quotation Failed");
                return CommonResponse.Ok(result);
            }
            catch (Exception ex)
            {
                _logger.LogError("Cancel of Purchase Quotation Failed");
                return CommonResponse.Error(ex);
            }
        }
    }
