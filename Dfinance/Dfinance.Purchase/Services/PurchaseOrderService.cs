...
namespace Dfinance.Purchase.Services
{
    public class PurchaseOrderService : IPurchaseOrderService
    {
        ...
        public CommonResponse CancelPurchaseOrder(int TransId, int PageId,string reason)
        {
            if (!_authService.IsPageValid(PageId))
            {
                return PageNotValid(PageId);
            ...
            if (!_authService.UserPermCheck(PageId, 5))
            {
                return PermissionDenied("cancel Purchase Order");
            }
            try
            {
                var result = _transactionService.CancelTransaction(TransId, reason);
                _logger.LogInformation("Cancel of Purchase Order Failed");
                return CommonResponse.Ok(result);
            }
            catch (Exception ex)
            {
                _logger.LogError("Cancel of Purchase Order Failed");
                return CommonResponse.Error(ex);
            }
        }
    }
