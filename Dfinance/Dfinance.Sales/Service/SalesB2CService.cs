...
{
    public class SalesB2CService:ISalesB2CService
    {
        ...
        public SalesB2CService(DFCoreContext context, IAuthService authService, IHostEnvironment hostEnvironment, IInventoryTransactionService transactionService,
        ...
        public CommonResponse SaveSalesB2C(PurchaseWithoutTaxDto dto, int pageId, int voucherId)
        {
            ...
            {
                return PermissionDenied("Save Sales(B2C)");
            }
            using (var transactionScope = new TransactionScope())
            {
                try
                {
                    ...
                    _logger.LogInformation("Sales(B2C) Saved Successfully");
                    return CommonResponse.Created("Sales(B2C) Saved Successfully");
                }
                catch
                {
                    transactionScope.Dispose();
                    _logger.LogError("Failed to Save Sales(B2C)");
                    transactionScope.Dispose();
                    return CommonResponse.Error("Failed to Save Sales(B2C)");
                }
            }
        }
        public CommonResponse UpdateSalesB2C(PurchaseWithoutTaxDto dto, int pageId, int voucherId)
        {
            if (!_authService.IsPageValid(pageId))
            {
                return PageNotValid(pageId);
            ...
            {
                return PermissionDenied("Update Sales(B2C)");
            }
            using (var transactionScope = new TransactionScope())
            {
                try
                {
                    ...
                    transactionScope.Complete();
                    _logger.LogInformation("Sales(B2C) Updated Successfully");
                    return CommonResponse.Created("Sales(B2C) Updated Successfully");
                }
                catch
                {
                    transactionScope.Dispose();
                    _logger.LogError("Failed to Update Sales(B2C)");
                    transactionScope.Dispose();
                    return CommonResponse.Error("Failed to Update Sales(B2C)");
                }
            }
        }
    }
