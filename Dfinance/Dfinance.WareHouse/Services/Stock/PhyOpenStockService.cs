...
namespace Dfinance.WareHouse.Services.Stock
{
    public class PhyOpenStockService : IPhyOpenStockService
    {
        private readonly DFCoreContext _context;
        private readonly IAuthService _authService;
        private readonly ILogger<PhyOpenStockService> _logger;
        private readonly IInventoryItemService _itemService;
        
        private readonly IUserTrackService _userTrackService;
       
        public PhyOpenStockService(DFCoreContext context, IAuthService authService, ILogger<PhyOpenStockService> logger,
        ...
        public CommonResponse SavePhyOpenStock(PhyOpenStockDto phyOpenStockDto, int pageId,int voucherId)
        {
            ...
            using (var transactionScope = new TransactionScope())
            {
                try
                {
                    ...
                    if ((VoucherType)primaryVoucherID == VoucherType.Physical_Stock || (VoucherType)primaryVoucherID == VoucherType.Opening_Stock)
                    {
                        var TransId = (int)SaveTransactions(phyOpenStockDto, pageId, voucherId).Data;
                        SaveTransactionAdditionals(phyOpenStockDto, TransId);
                        _itemService.SaveInvTransItems(phyOpenStockDto.Items, voucherId, TransId,phyOpenStockDto.ExchangeRate,phyOpenStockDto.Warehouse.Id);
                    }
                    else { return CommonResponse.Error("Invalid voucherId"); }
        ...
        public CommonResponse UpdatePhyOpenStock(PhyOpenStockDto phyOpenStockDto, int pageId, int voucherId)
        {
            if (!_authService.IsPageValid(pageId))
            {
                return PageNotValid(pageId);
            ...
            using (var transactionScope = new TransactionScope())
            {
                try
                {
                    ...
                    if ((VoucherType)primaryVoucherID == VoucherType.Physical_Stock || (VoucherType)primaryVoucherID == VoucherType.Opening_Stock)
                    {
                        var TransId = (int)SaveTransactions(phyOpenStockDto, pageId, voucherId).Data;
                        SaveTransactionAdditionals(phyOpenStockDto, TransId);
                        _itemService.UpdateInvTransItems(phyOpenStockDto.Items, voucherId, TransId, phyOpenStockDto.ExchangeRate, phyOpenStockDto.Warehouse.Id);
                    }
                    else { return CommonResponse.Error("Invalid voucherId"); }
                    transactionScope.Complete();
        ...
        private CommonResponse SaveTransactions(PhyOpenStockDto phyOpenStockDto, int pageId, int voucherId)
        {
            ...
            if (phyOpenStockDto.Id == 0 || phyOpenStockDto.Id == null)
            {
                reason = "Added";
                var voucherNoCheck = _context.FiTransaction.Any(t => t.VoucherId == voucherId && t.TransactionNo == phyOpenStockDto.VoucherNo);
                if (voucherNoCheck)
                ...
                _context.Database.ExecuteSqlRaw("EXEC VoucherSP @Criteria={0}, @Date={1}, @EffectiveDate={2}, @VoucherID={3}, @MachineName={4}, " +
                               ...
                               criteria, phyOpenStockDto.Date, phyOpenStockDto.Date, voucherId, machinename, phyOpenStockDto.VoucherNo, IsPostDated, null, ExchangeRate,
            ...
            else
            {
                actionId = 1;
                reason = "Updated";
                int Transaction = _context.FiTransaction.Where(x => x.Id == phyOpenStockDto.Id).Select(x => x.Id).FirstOrDefault();
                if (Transaction == null)
                {
                ...
                _context.Database.ExecuteSqlRaw("EXEC VoucherSP " +
                    ...
                    "@RefTransID={33}, @CostCentreID={34}, @PageID={35}, @ID={36}",
                            criteria, phyOpenStockDto.Date, phyOpenStockDto.Date, voucherId, machinename, phyOpenStockDto.VoucherNo, IsPostDated, null, ExchangeRate,
                               null, null, null, branchId, null, null, null, null, null, null, userId, null, AddedDate, null,
                               ApprovalStatus, null, null, Status, IsAutoEntry, Posted, Active, Cancelled, accId, null, null, null, pageId, phyOpenStockDto.Id);

                transId = (int)phyOpenStockDto.Id;

            }
            decimal amount = 0;
            var jsonBudget = JsonSerializer.Serialize(phyOpenStockDto);
            int primaryvoucherId = (from v in _context.FiMaVouchers
                                    join pm in _context.MaPageMenus on v.Id equals pm.VoucherId
                                    where pm.Id == pageId
                                    select v.PrimaryVoucherId).FirstOrDefault() ?? 0;
            string modulename = null;
            if ((VoucherType)primaryvoucherId == VoucherType.Physical_Stock)
            {
                 modulename = "Physical Stock";
            }
            else if ((VoucherType)primaryvoucherId == VoucherType.Opening_Stock) { modulename = "Opening Stock"; }
           _userTrackService.AddUserActivity(phyOpenStockDto.VoucherNo, transId, actionId, reason, "FiTransactions", modulename, amount, jsonBudget);
            return CommonResponse.Ok(transId);
        }
        private CommonResponse SaveTransactionAdditionals(PhyOpenStockDto phyOpenStockDto, int TransId)
        {
            string criteria = "";
        ...
        "@BranchID={86}, @AreaID={87}, @TaxFormID={88}, @PriceCategoryID={89}, @IsClosed={90}, @DepartmentID={91}, @PartyName={92}, @Address1={93}, @Address2={94}, @ItemID={95}, @VATNo={96}",
                ...
                null,//8
                null,//9
                phyOpenStockDto.Warehouse.Id,//10
                ...
                null,//64
                null,//65
                null, phyOpenStockDto.Terms, null,
                null,//69
                null, null, null, null,
                null,//74
                null,
                null,//76
                null,
                phyOpenStockDto.Warehouse.Id,//78
                null,//79
                null, null,
                null,//82
                null, null, null, null,
